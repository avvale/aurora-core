...
export abstract class DecimalValueObject extends NumberValueObject {
  set value(value: number) {
    if (!(this.validationRules.undefinable && value === undefined)) {
      ...
      const integersLimit =
        this.validationRules.decimals[0] - this.validationRules.decimals[1];
      const decimalsLimit = this.validationRules.decimals[1];

      if (decimalCounter.integers > integersLimit)
        throw new BadRequestException(
          `Value for ${this.validationRules.name} is too large, has a maximum length of ${integersLimit} integers in ${value} number`,
        );
      if (decimalCounter.decimals > decimalsLimit)
        throw new BadRequestException(
          `Value for ${this.validationRules.name} is too large, has a maximum length of ${decimalsLimit} decimals in ${value} number`,
        );
    }
  ...
  get value(): number {
    return typeof super.value === 'number'
      ? super.value
      : Number.isNaN(Number.parseFloat(super.value))
        ? super.value
        : Number.parseFloat(super.value);
  }
