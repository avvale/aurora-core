...
export abstract class ValueObject<T> implements IValueObject<T> {
  ...
  set value(value: T) {
    ...
    if (
      this.validationRules.nullable === false &&
      value === null &&
    ...
    if (
      this.validationRules.undefinable === false &&
      value === undefined &&
      !this.validationRules.default
    ) {
      throw new BadRequestException(
        `Value for ${this.validationRules.name} must be defined, can not be undefined`,
      );
    }
  ...
  constructor(
    dataValue: T | DataValueObject | undefined,
    validationRules: ValidationRules = {},
    data: DataValueObject | T = {},
  ) {
    // set default value
    let value = undefined;
