...
export abstract class NumberValueObject extends ValueObject<number | null | undefined>
{
    set value(value: number | null | undefined)
    {
        ...
        if (this.validationRules.unsigned && Math.sign(value) === -1)
        {
            ...
            throw new BadRequestException(`The numerical value for ${this.validationRules.name} must have a positive sign, this field does not accept negative values, the received value is ${value}`);
        }
