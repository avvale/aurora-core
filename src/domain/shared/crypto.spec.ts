...
describe('Crypt', () => {
  ...
  describe('encrypt / decrypt', () => {
    ...
    test('fails to decrypt when payload is tampered', () => {
      ...
      expect(() =>
        Crypt.decrypt(payloadParts.join(':'), recipientPrivateKey),
      ).toThrow();
    });
  });
  ...
  describe('sign / verifySignature', () => {
    test('verifies a valid signature', () => {
      ...
      expect(
        Crypt.verifySignature(message, signature, recipientPublicKey),
      ).toBe(true);
    });
    ...
    test('rejects a signature if message changes', () => {
      ...
      expect(
        Crypt.verifySignature('Altered message', signature, recipientPublicKey),
      ).toBe(false);
    });
    ...
    test('rejects signatures produced by a different key pair', () => {
      ...
      expect(Crypt.verifySignature(message, signature, otherPublicKey)).toBe(
        false,
      );
    });
  });
