...
export class ImageManager {
  public static async dimensions(
    path: string,
  ...
  public static async exif(prop: string | Buffer) {
    const buffer =
      prop instanceof Buffer ? prop : (await sharp(prop).metadata()).exif;
  ...
  public static crop(image: sharp.Sharp, region: sharp.Region): sharp.Sharp {
  ...
  public static cropAttachment(attachment, attachmentFamily, crop) {
    const image = sharp(
      path.join(attachment.library.pathname, attachment.library.filename),
    );
