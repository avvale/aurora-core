...
export class CoreAddI18nConstraintService {
  ...
  public async add(
    ...
    if (!lang && defineDefaultLanguage) {
      lang = langs.find(
        (lang) =>
          lang[CoreSearchKeyLang.ISO6392] ===
          this.configService.get<string>('APP_FALLBACK_LANG'),
      );
    }
    ...
    if (!lang && defineDefaultLanguage) {
      throw new InternalServerErrorException(
        'APP_FALLBACK_LANG must be defined in iso6392 lang code format in .env file, not found contentLanguage: ',
        contentLanguage,
      );
    }
