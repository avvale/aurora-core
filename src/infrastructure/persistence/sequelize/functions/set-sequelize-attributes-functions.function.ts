...
type AggSpec = {
  ...
  args?: Array<
    string | number | { col: string } | { fn: string; args?: any[] }
...
function buildExprFromArg(arg: any): any {
  if (arg == null || typeof arg === 'number' || typeof arg === 'string')
    return arg;
...
function buildAggTuple(spec: AggSpec): any[] {
  ...
  if (!args.length && spec.col) {
    ...
    const firstArg = spec.distinct
      ? Sequelize.fn('DISTINCT', baseCol)
      : baseCol;
    args = [firstArg];
...
export function transformAttributes(attributes: any) {
  ...
  if (Array.isArray(attributes)) {
    return attributes.map((item) =>
      isAggSpec(item) ? buildAggTuple(item) : item,
    );
  }
  ...
  if (
    ...
    Array.isArray(attributes.include)
  ) {
    return {
      ...attributes,
      include: attributes.include.map((item: any) =>
        isAggSpec(item) ? buildAggTuple(item) : item,
      ),
    };
  }
