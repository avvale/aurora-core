...
import { CQMetadata, DateTime, LiteralObject, QueryStatement } from '../../../..';
dayjs.extend(utc);
...
): LiteralObject =>
{
    ...
    {
        return currentStatement.map(val => setSequelizeFunctions(val as unknown as QueryStatement, cQMetadata, options));
    }
    ...
    for (const key of Reflect.ownKeys(currentStatement))
    {
            ...
            for (const [index, fn] of functions.entries())
            {
                switch (fn)
                {
                    ...
                    case 'unaccent':
                        if (isPlainObject(value) || Array.isArray(value))
                        {
                            ...
                            currentStatement[rawColumn] = Sequelize.where(
                                ...
                                setSequelizeFunctions(value, cQMetadata, { ...options, setUnaccentValues: true }),
                            );
                        }
                        else
                        {
                    ...
                    case 'timestamp':
                        if (isPlainObject(value) || Array.isArray(value))
                        {
                            currentStatement[rawColumn] = setSequelizeFunctions(value as QueryStatement, cQMetadata, { ...options, setTimestamp: true });
                        }
                        else
                        {
        ...
        if (!isPlainObject(value) && !Array.isArray(value))
        {
            ...
            if (DateTime.isTimestamp(value) && cQMetadata?.timezone) currentStatement[key] = dayjs.tz(value as string, cQMetadata.timezone).tz(process.env.TZ ?? (dayjs.tz.guess?.() ?? 'UTC')).format('YYYY-MM-DD HH:mm:ss');
        }
        else
        {
