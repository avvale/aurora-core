...
export class SequelizeAuditingAgent {
  public static registerSideEffect(
    ...
    if (!auditingMeta) {
      Logger.log(
        `Event ${event} of ${modelName} model, is not audited`,
        SequelizeAuditingAgent.name,
      );
      return;
    }
    ...
    if (
      ...
      if (!('dataValues' in instance) || !('_previousDataValues' in instance))
        throw new BadRequestException(
          `The instance for hook with event ${event} ane model ${modelName} does not defined.`,
        );

      dataValues = _.omit(instance.dataValues, [
        'createdAt',
        'updatedAt',
        'deletedAt',
      ]);
      previousDataValues = _.omit(instance._previousDataValues, [
        'createdAt',
        'updatedAt',
        'deletedAt',
      ]);
    ...
    if (event === AuditingSideEffectEvent.BULK_CREATED) {
      if (
        (Array.isArray(instance) &&
          instance.length > 0 &&
      ...
      dataValues = instance.map((item) =>
        _.omit(item.dataValues, ['createdAt', 'updatedAt', 'deletedAt']),
      );
      previousDataValues = instance.map((item) =>
        _.omit(instance._previousDataValues, [
          'createdAt',
          'updatedAt',
          'deletedAt',
        ]),
    ...
    switch (event) {
      ...
      case AuditingSideEffectEvent.UPDATED:
        newValue = Utils.diff(
          SequelizeAuditingAgent.parseObject(dataValues),
          SequelizeAuditingAgent.parseObject(previousDataValues),
        );
        oldValue = Utils.diff(
          SequelizeAuditingAgent.parseObject(previousDataValues),
          SequelizeAuditingAgent.parseObject(dataValues),
